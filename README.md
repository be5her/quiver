...
- `.quiver/local/`: gitignored. UI state, request and query history, requests captured by mock servers, realtime message logs, MCP traffic logs, fetched GraphQL schemas, and secrets (environment values, database passwords) encrypted with the OS keychain.
